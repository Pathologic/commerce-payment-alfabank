...
Перед началом работы необходимо обратиться в поддержку и запросить включение уведомлений: callback-уведомления с контрольной суммой с симметричной криптографией. Поддержка может уточнить URL для отправки уведомлений - плагин отправляет его в запросе, поэтому можете сообщить просто URL сайта. Поддержка предоставит криптографический ключ (токен).
...
Перечень тестовых карт для тестирования сервиса оплаты: <https://alfabank.ru/sme/payservice/internet-acquiring/docs/connection-options/api/test-cards/>